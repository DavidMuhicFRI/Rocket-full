...
namespace RocketSim
{
    ...
    public sealed class LandingGearCollider : MonoBehaviour
    {
        ...
        void OnCollisionEnter(Collision collision) => RelayCollision(collision, initialContact: true);
        void OnCollisionStay(Collision collision) => RelayCollision(collision, initialContact: false);
        ...
        /// </summary>
        void RelayCollision(Collision collision, bool initialContact)
        {
            FalconAgent owner = Owner;
            if (owner)
                owner.RecordLandingGearCollision(collision, this, initialContact);
        }
    }
}
