...
The long-term thesis goal is to study how reinforcement learning algorithms such as Proximal Policy Optimization (PPO) and Soft Actor-Critic (SAC) perform on rocket control tasks. Planned experiments include hovering, takeoff, landing, stage-separation-style attitude control, wind disturbances, possible hardware defects, and different rocket configurations. The current document describes the simulator and the systems that have already been implemented. The final experimental results and the full development story will be added later.
...
\begin{itemize}
    ...
    \item diagnostic hardware-style tests for engines, fins, aerodynamics, RCS, takeoff, and landing behavior.
\end{itemize}
...
The current simulator supports five scenario types:
...
\midrule
\endhead
Landing & The rocket starts above the pad with downward velocity and must descend upright, slow down, and touch down near the target. \\
...
Takeoff & The rocket starts near ground clearance and must lift off, climb, and avoid excessive lateral drift. \\
Belly Flop & The rocket starts in a high-tilt state and must transition toward upright landing behavior. \\
\bottomrule
\caption{Implemented scenario modes.}
\end{longtable}

Hover and hover tracking use a goal altitude of \(30\,\mathrm{m}\). Takeoff uses a goal altitude of \(120\,\mathrm{m}\). Landing-style scenarios use a ground-clearance target of \(0.5\,\mathrm{m}\).
...
Landing rewards low horizontal error, upright attitude, low angular rate, low speed, and low vertical speed near the landing zone. Hover rewards altitude control, horizontal centering, low speed, and stable attitude. Hover tracking separates the task into a movement phase and a hover phase. Takeoff rewards altitude gain, upward velocity, and uprightness. Belly-flop mode rewards broadside descent at altitude and upright landing behavior near the ground.
